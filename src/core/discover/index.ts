...
import { loadPatternRegistry, BUILTIN_PATTERNS, isBuiltinModule } from './patterns';
import { detectLanguages, filterPatterns } from './detect';
import type {
  LanguagePattern,
  DiscoveredEdge,
  AmbiguousRelation,
...
interface RawReference {
  ...
  target_name: string;
  language: string;
  strategy: 'source_import' | 'dependency_file';
  confidence: number;
  matched_pattern: string;
  /**
   * 'local_file' = target was a path we tried to resolve to a project file.
   * 'external_bare' = target was a bare name (npm pkg, stdlib, etc.) or a
   *                   full path from a dep file (Cargo.toml dependency).
   * The distinction drives the `unmatched_target` vs `external_unmatched`
   * classification so agents only see actionable items.
   */
  target_kind: 'local_file' | 'external_bare';
}
...
export function discoverCommand(options: {
  ...
  // 1. Load patterns
  let extraPatterns: LanguagePattern[] | undefined;
  if (options.patternFile) {
    try {
      const raw = readFile(path.resolve(cwd, options.patternFile));
      if (raw) {
  ...
  const allPatterns = loadPatternRegistry(extraPatterns);

  // 2. Detect or filter languages
  let activePatterns: LanguagePattern[];
  if (options.lang && options.lang !== 'auto') {
    const langs = options.lang.split(',').map(l => l.trim().toLowerCase());
    activePatterns = filterPatterns(allPatterns, langs);
  } else {
    const detected = detectLanguages(cwd, allPatterns);
  ...
  const sourceFileRows = db.prepare(
    "SELECT DISTINCT path, card_id FROM paths WHERE relation = 'source_file'"
  ).all() as Array<{ path: string; card_id: string }>;

  // Build: absolutePath → cardId map; also track scanned files to avoid duplicates
  const fileCardMap = new Map<string, string>();
  ...
  for (const langPattern of activePatterns) {
    ...
    for (const [filePath, cardId] of fileCardMap) {
      ...
      if (scannedFiles.has(absPath)) continue;
      ...
      for (const pattern of langPattern.source_patterns) {
        ...
        try {
          ...
          while ((match = re.exec(content)) !== null) {
            ...
            if (isBuiltinModule(cleanTarget, langPattern.language)) continue;
    ...
    for (const depFile of langPattern.dep_files) {
      if (depFile.confidence < minConfidence) continue;
      const depPath = path.join(cwd, depFile.filename);
      if (!fileExists(depPath)) continue;
      ...
      for (const dep of deps) {
        ...
        if (isBuiltinModule(dep, langPattern.language)) continue;
        allRefs.push({
          from_file: depPath,
  ...
  if (!options.dryRun) {
    ...
    for (const cardId of dirtyCardIds) {
      insertDirtyFlag(db, 'card', cardId, 'discover:inferred_edges_added', activeSession?.id);
    }
 ...
 */
function resolveToCards(
  refs: RawReference[],
  fileCardMap: Map<string, string>,
  db: ReturnType<typeof openDatabase>,
  minConfidence: number,
): { edges: DiscoveredEdge[]; ambiguous: AmbiguousRelation[] } {
  ...
  for (const ref of refs) {
    ...
    if (!fromId) continue;
    if (fromId === toId) continue; // self-reference
...
function findTargetCards(
  ...
  if (fileCardMap.has(target)) {
    ...
    if (!results.includes(cid)) results.push(cid);
  }
  ...
  if (looksLikePath) {
    ...
    for (const [fpath, cid] of fileCardMap) {
      if (results.includes(cid)) continue;
  ...
  try {
    const aliasRows = db.prepare(
      'SELECT card_id FROM aliases WHERE normalized_alias = ?'
    ).all(target.toLowerCase().trim()) as Array<{ card_id: string }>;
  ...
  try {
    ...
    const cardRows = db.prepare(
      'SELECT id FROM cards WHERE id = ? AND is_deleted = 0'
    ).all(normalizedTarget) as Array<{ id: string }>;
...
function extractDeps(
  content: string,
  depFilePattern: { parser: string; extractDeps: string },
): string[] {
  const deps: string[] = [];
      ...
      if (extractKey === 'line') {
        ...
        if (blockMatch) {
          for (const line of blockMatch[1].split('\n')) {
            ...
            if (parts.length >= 1 && parts[0]) deps.push(parts[0]);
          }
        }
        // Single-line require
 ...
 */
function dedupAmbiguous(items: AmbiguousRelation[]): AmbiguousRelation[] {
  const seen = new Set<string>();
  const result: AmbiguousRelation[] = [];
 ...
 */
function printCompact(result: DiscoverResult): void {
