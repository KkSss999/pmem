 ...
 */
export const BUILTIN_MODULES: Record<string, Set<string>> = {
  nodejs: new Set([
 ...
 */
export function isBuiltinModule(target: string, language: string): boolean {
  const set = BUILTIN_MODULES[language];
  if (!set) return false;
  ...
  if (language === 'java' || language === 'python') {
    ...
    if (set.has(firstSegment)) return true;
 ...
 */
export const BUILTIN_PATTERNS: Record<string, LanguagePattern> = {
  nodejs: {
    language: 'nodejs',
...
export function loadPatternRegistry(
  manifestPatterns?: LanguagePattern[],
): LanguagePattern[] {
  const merged = { ...BUILTIN_PATTERNS };
